...
print "Promedio exploracion: ", promedio/100

Q = np.zeros((height * width, num_actions))  # Q matrix

def egreedy(state, alfa):
    valorMaximo = 0
    mejorAccion = 0
    acciones = getActions(state)
    for accion in acciones:
        if (valorMaximo < Q[state][actions_list[accion]]):
            valorMaximo = Q[state][actions_list[accion]]
            mejorAccion = accion

    if (valorMaximo == 0 or random.randrange(0,100) <= alfa):
...
print "Promedio egreedy: ", promedio/100
